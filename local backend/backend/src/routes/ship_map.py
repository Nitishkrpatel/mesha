...
class Search(Resource):
    def post(self):
        ...
        elif criteria == 'anchor':
            return self.anchorage_search(text)
        else:
    ...
    @token_required
    def anchorage_search(self, anchorage_name, _):
        anchorge_search_query = "SELECT label FROM anchors WHERE label ILIKE %s"
        with shipdb_pool.connect() as conn:
            df = pd.read_sql_query(anchorge_search_query, conn, params=(anchorage_name + "%",))
        df.columns = ['anchor']
        if len(df) == 0:
            msg = """Particular anchor doesnt exists!"""
            return make_response(jsonify({"status": "failure", "message": msg}), 401)
        df = df.to_json(orient='records')
        data = json.loads(df)
        return make_response(jsonify({"status": "success", "data": data}), 200)
    ...
    def ship_search_results(self, search_txt, timestamp, criteria, user_name):
        try:
            ...
            if criteria == 'MMSI':
                ...
                self.store_search(criteria, search_txt, user_name)
                port_search_query = "select lat,long from anchors where label=%s limit 1"
                with shipdb_pool.connect() as connection:
                    data = pd.read_sql_query(port_search_query, connection, params=(search_txt,))
                data = data.to_json(orient='records')
                data = json.loads(data)
...
# All anchor coordinates for plotting
class AnchorsDetails(Resource):
    def get(self):
        return self.anchors()

    @token_required
    def anchors(self):
        anchors_query = """select lat,long,label,sublabel,country_name from anchors"""
        with shipdb_pool.connect() as connection:
            result = connection.execute(anchors_query,)
            anchors_data = result.fetchall()
        data = pd.DataFrame(anchors_data)
        data = data.to_json(orient='records')
        data = json.loads(data)
        return make_response(jsonify({"status": "success", "data": data}), 200)
...
api.add_resource(AnchorsDetails, "/ship_map/anchors")
